  ...
  return <div className="app" onKeyDown={(e) => {
    ...
    {profile.session.lastScreen === 'operations-overview' && <section>
      ...
      {[{ key: '+', title: tr.additionUpToTwenty }, { key: '-', title: tr.subtractionUpToTwenty }].map((operation) => <div key={operation.key} className="overview-block">
        ...
        <table>
          <thead><tr>{Array.from({ length: 21 }, (_, value) => <th key={`head-${operation.key}-${value}`}>{value}</th>)}</tr></thead>
          <tbody><tr>{Array.from({ length: 21 }, (_, value) => {
            const key = `${operation.key}:${value}`;
            ...
            const title = `${tr.resultHeader}: ${value} • ${tr.statsTooltipAttempts}: ${stats.attempts} • ${tr.statsTooltipCorrect}: ${stats.correct} • ${tr.statsTooltipWrong}: ${stats.wrong}`;
            return <td key={`result-${operation.key}-${value}`} className="overview-cell" style={{ background: bg, color }} title={title}>{value}</td>;
          })}</tr></tbody>
        </table>
      </div>)}
    </section>}
