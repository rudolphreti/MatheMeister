...
const defaultSettings: Settings = { mode: 'timed', sessionMinutes: 10, min: 0, max: 20, additionEnabled: true, subtractionEnabled: true, subtractionMinuendMax: 20, terms: 2, soundEnabled: true, language: 'de', examplesPerSession: 10, excludeResultZero: false, excludePlusMinusZero: false, excludePlusMinusOne: false, customTasksText: '' };
  ...
  return <div className="app" onKeyDown={(e) => {
    ...
    {profile.session.lastScreen === 'settings' && <section>
      ...
      <label>{tr.maxLabel} <select value={profile.settings.max} onChange={(e) => setProfile((p) => {
        ...
        return { ...p, settings: { ...p.settings, max: nextMax, subtractionMinuendMax: Math.min(nextMax, p.settings.subtractionMinuendMax) } };
      ...
      <fieldset>
        ...
        {profile.settings.subtractionEnabled && <label>{tr.subtractionMinuendMaxLabel} <input type="number" min={profile.settings.min} max={profile.settings.max} step={1} value={profile.settings.subtractionMinuendMax} onChange={(e) => {
          const value = Number(e.target.value);
          if (!Number.isFinite(value)) return;
          setProfile((p) => ({ ...p, settings: { ...p.settings, subtractionMinuendMax: Math.max(p.settings.min, Math.min(p.settings.max, Math.floor(value))) } }));
        }} /></label>}
      </fieldset>
