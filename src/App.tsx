...
export function App() {
  ...
  useEffect(() => {
    if (!ended) return;
    if (Object.keys(pendingProblemStats).length === 0) return;
    setProfile((p) => ({ ...p, problemStats: mergeProblemStats(p.problemStats, pendingProblemStats) }));
    setPendingProblemStats({});
  }, [ended, pendingProblemStats]);
